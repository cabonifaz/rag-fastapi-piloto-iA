...
class AreaRepository:
    ...
    def get_areas_paginated(
        ...
        try:
            ...
            try:
                cursor.execute(
                    "EXEC SP_AREAS_LST_PAG @NUM_PAGINA = ?, @TAM_PAGINA = ?, @TERM_BUSQUEDA = ?, @CAMPO_ORDEN = ?, @DIR_ORDEN = ?, @ID_USUARIO = ?, @ID_EMPRESA = ?, @FILTRO_ESTADO = ?",
                    num_pagina,
                    tam_pagina,
                    term_busqueda,
                    campo_orden,
                ...
                while True:
                    try:
                        ...
                        if cursor.description:
                            ...
                            # Check if this result set contains AREA data (not role data)
                            has_area_data = 'ID_AREA' in columns and 'AREA' in columns
                            ...
                            if has_message_columns and rows:
                            ...
                            elif has_area_data and rows:  
                                # This is the area data with pagination
                                ...
                                for row in rows:
                                    ...
                                    area_dict.pop('TotalRecords', None)
                                    area_dict.pop('CurrentPage', None)
                                    area_dict.pop('PageSize', None)
                                    area_dict.pop('TotalPages', None)
                ...
                logger.error(f"Cursor error in get_areas_paginated: {cursor_error}")
                cursor.close()
                self.db.rollback()
                raise

        except Exception as e:
            logger.error(f"Error fetching paginated areas with SP: {e}")
            self.db.rollback()
