"""
LangGraph workflow for LLM-only query processing (no RAG).
...
from app.workflows.nodes.conversation_nodes import create_get_conversation_history_node
from app.workflows.nodes.preprocessing_nodes import create_clean_message_node, create_load_rag_config_node
from app.workflows.nodes.chat_nodes import create_save_user_message_node
...
_compiled_llm_only_workflow: Optional[Any] = None


def create_llm_only_workflow(
    session_factory: Any,
    message_service: Any,
    ia_config_service: Any
) -> StateGraph:
    """
    Create and configure the LLM-only workflow graph (no RAG).
    ...
    get_conversation_history = create_get_conversation_history_node(message_service, max_ctx=8)  # Max 8 for LLM-only
    ...
    save_user_message = create_save_user_message_node(message_service)  # Reuse (respects store_messages flag)
    ...
    workflow.add_edge("load_config", "save_user_message")
...
def initialize_llm_only_workflow(
    session_factory: Any,
    message_service: Any,
    ia_config_service: Any
) -> None:
    """
    Initialize and compile the global LLM-only workflow.
    ...
    Args:
        ...
        ia_config_service: IA config service
    """
    global _compiled_llm_only_workflow

    logger.info("Compiling LLM-only workflow...")
    workflow = create_llm_only_workflow(
        session_factory=session_factory,
        message_service=message_service,
        ia_config_service=ia_config_service
    )
    _compiled_llm_only_workflow = workflow.compile()
    logger.info("LLM-only workflow compiled successfully")


def get_compiled_llm_only_workflow() -> Any:
    """
    Get the compiled LLM-only workflow.
    Raises ValueError if workflow hasn't been initialized.

    Returns:
        Compiled StateGraph ready to execute
    """
    if _compiled_llm_only_workflow is None:
        raise ValueError("LLM-only workflow not initialized. Call initialize_llm_only_workflow() at startup.")
    return _compiled_llm_only_workflow
